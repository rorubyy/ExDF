...
from lavis.models.blip2_models.prompt_learner import PromptLearner
...
class Blip2VicunaInstruct(Blip2Base):
    ...
    def forward(self, samples):
        ...
        with self.maybe_autocast():
            ...
            gt_embeds = self.ln_vision(self.visual_encoder(gt))
        combined_embeds = image_embeds + gt_embeds
        image_atts = torch.ones(combined_embeds.size()[:-1], dtype=torch.long).to(
            image.device
        )

        bs = image.size(0)

        query_tokens = self.query_tokens.expand(combined_embeds.shape[0], -1, -1)
